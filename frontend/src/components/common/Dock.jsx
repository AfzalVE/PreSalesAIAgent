    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={`${className} absolute -top-10 left-1/2 w-fit whitespace-pre rounded-lg border border-neutral-200/60 bg-white/95 px-2.5 py-1 text-[10px] font-bold text-neutral-700 shadow-sm`}
          role="tooltip"
          style={{ x: '-50%' }}
        >
          {children}
  ...
  return (
    <motion.div style={{ height, scrollbarWidth: 'none' }} className="mx-2 flex max-w-full items-center">
      <motion.div
        ...
        className={`${className} fixed bottom-6 left-1/2 transform -translate-x-1/2 flex items-end w-fit gap-3 rounded-2xl border-neutral-200/50 bg-white/80 backdrop-blur-md border pb-1.5 px-3.5 shadow-lg z-50`}
